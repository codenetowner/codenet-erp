...
// Set content root to exe directory (important when launched from Startup folder)
...
// Initialize database (synchronous - blocks until ready)
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        ...
        using (var adminConn = new Npgsql.NpgsqlConnection(connBuilder.ConnectionString))
        {
            ...
            if (!exists)
            {
                Console.WriteLine($"[DB Init] Database '{databaseName}' does not exist. Creating...");
                using var createCmd = adminConn.CreateCommand();
                createCmd.CommandText = $"CREATE DATABASE \"{databaseName}\"";
                createCmd.ExecuteNonQuery();
                Console.WriteLine($"[DB Init] Database '{databaseName}' created!");
            }
        }
        
        // Create tables from EF model
        context.Database.EnsureCreated();
        Console.WriteLine("[DB Init] Database tables created/verified successfully");
        ...
        if (companyCount == 0)
        {
            Console.WriteLine("[DB Init] No companies found. Seeding default data...");
            ...
            if (companyId > 0)
            {
                ...
                Console.WriteLine("[DB Init] DEFAULT SETUP COMPLETE! Login: admin / 123456");
            }
        }
        ...
        Console.WriteLine("[DB Init] Database ready!");
    }
    catch (Exception ex)
    {
...
// Use PORT environment variable for Railway, fallback to 5227 for local development
